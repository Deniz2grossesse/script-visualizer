...
// Fonction pour générer des scripts à partir des données
function generateScripts(data) {
  try {
    console.log("generateScripts called with data:", JSON.stringify(data));
    const rules = data.csvRows || [];
...
function saveDraft(formData) {
  try {
    ...
    userProperties.setProperty('draftData', JSON.stringify(formData));
    return {
      success: true,
    ...
    return {
      success: false,
      message: "Erreur lors de la sauvegarde du brouillon: " + e.toString()
    };
  }
}
...
function getDraft() {
  try {
    const userProperties = PropertiesService.getUserProperties();
    const draftData = userProperties.getProperty('draftData');
    ...
    return {
      success: false,
      message: "Erreur lors de la récupération du brouillon: " + e.toString()
    };
  }
}

// Fonction pour supprimer le formulaire et les données associées
function deleteForm() {
  try {
    const userProperties = PropertiesService.getUserProperties();
    userProperties.deleteProperty('draftData');
    headerLinesCache = [];
